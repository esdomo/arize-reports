# Batch Report System - Implementation Guide

**Date:** 2025-11-19
**Status:** Ready for Implementation
**Companion to:** 2025-11-19-batch-report-system.md

---

## Implementation Overview

This document provides detailed code structure, method signatures, and examples for implementing the batch report system.
...
```
arize-reports/
  batch_reporter.py          # Main BatchReporter class
  session_analyzer.py        # Existing analyzer (unchanged)
  requirements.txt           # Dependencies (unchanged)
  batch_outputs/             # Batch report output directory
    2025-11-19_daily/
      ...
```
...
class BatchReporter:
    ...
    def run_batch(
        ...
        print(f"Found {len(df)} spans across {df['session.id'].nunique() if 'session.id' in df.columns else 'unknown'} sessions")
    ...
    def _session_summary_report(
        ...
        if "session.id" in df.columns:
            session_df = df.groupby("session.id").agg(
                span_count=("context.span_id", "count") if "context.span_id" in df.columns else ("session.id", "count"),
                first_activity=("start_time", "min") if "start_time" in df.columns else ("session.id", "first"),
                last_activity=("end_time", "max") if "end_time" in df.columns else ("session.id", "last")
            ).reset_index()
    ...
    def _conversation_export(
        ...
        # Use existing conversation extraction logic
        conversations = []
        ...
        return {
            ...
            "sessions_included": conv_df["session_id"].nunique() if "session_id" in conv_df.columns else 0
        }
    ...
    def _cost_analysis_report(
        ...
        if "llm.model_name" in df.columns:
            for model_name in df["llm.model_name"].dropna().unique():
                ...
                prompt = pd.to_numeric(model_df.get("llm.token_count.prompt", 0), errors='coerce').fillna(0).sum()
                completion = pd.to_numeric(model_df.get("llm.token_count.completion", 0), errors='coerce').fillna(0).sum()
        ...
        if "llm.provider" in df.columns:
            for provider in df["llm.provider"].dropna().unique():
                ...
                provider_tokens = int(pd.to_numeric(provider_df.get("llm.token_count.total", 0), errors='coerce').fillna(0).sum())
        ...
        if "llm.model_name" in df.columns and cache_read > 0:
            for model_name in df["llm.model_name"].dropna().unique():
                model_df = df[df["llm.model_name"] == model_name]
                model_cache = pd.to_numeric(model_df.get("llm.token_count.prompt_details.cache_read", 0), errors='coerce').fillna(0).sum()
        ...
        if "status_code" in df.columns:
            ...
            failed_tokens = int(pd.to_numeric(failed_df.get("llm.token_count.total", 0), errors='coerce').fillna(0).sum())
            ...
            if "llm.model_name" in failed_df.columns:
                for model_name in failed_df["llm.model_name"].dropna().unique():
                    ...
                    prompt = pd.to_numeric(model_failed.get("llm.token_count.prompt", 0), errors='coerce').fillna(0).sum()
                    completion = pd.to_numeric(model_failed.get("llm.token_count.completion", 0), errors='coerce').fillna(0).sum()
            ...
            for session_id in df["session.id"].dropna().unique():
                ...
                if "llm.model_name" in session_df.columns:
                    for model_name in session_df["llm.model_name"].dropna().unique():
                        ...
                        prompt = pd.to_numeric(model_session.get("llm.token_count.prompt", 0), errors='coerce').fillna(0).sum()
                        completion = pd.to_numeric(model_session.get("llm.token_count.completion", 0), errors='coerce').fillna(0).sum()
    ...
    def _top_sessions_report(
        ...
        # Top by tokens
        if "total_tokens" in metrics_df.columns:
        ...
        if "error_count" in metrics_df.columns:
...
# Run daily report (last 24 hours)
results = reporter.run_window("daily")

# Run weekly report
results = reporter.run_window("weekly")

# Custom time range
end = datetime.now()
start = end - timedelta(hours=6)
results = reporter.run_batch(start, end)

# Specific reports only
results = reporter.run_batch(
    start, end,
    reports=["session_summary", "cost"]
)
```
...
# Specific reports
python batch_reporter.py --reports "session_summary,cost,engagement"

# Specific project
python batch_reporter.py --project "my-project" --window daily
```
...
# Weekly report on Sunday at 1am
0 1 * * 0 /path/to/python /path/reporter.py --window weekly

# Hourly report
0 * * * * /path/to/python /path/to/batch_reporter.py --window hourly
```
...
```json
{
  ...
  "tokens": {
    ...
    "total_cache_read_tokens": 180000
  },
  "by_model": {
    "gpt-4o": {
      "spans": 120,
...
```
session_id,trace_id,timestamp,human_message,ai_response,model,tokens
sess-001,trace-001,2025-11-18T10:15:00,"Hello","Hi there!",gpt-4o,125
sess-001,trace-002,2025-11-18T10:16:00,"Help me","Sure...",gpt-4o,450
```
...
def test_batch_reporter_init():
    """Test BatchReporter initialization."""
    reporter = BatchReporter()
    assert reporter.analyzer is not None

def test_session_summary():
    """Test session summary report generation."""
    reporter = BatchReporter()
    end = datetime.now()
    start = end - timedelta(hours=1)

    results = reporter.run_batch(start, end, reports=["session_summary"])

    assert "reports" in results
    assert "session_summary" in results["reports"]
    assert "metrics" in results["reports"]["session_summary"]
...
def test_all_reports():
    ...
    expected_reports = ["session_summary", "conversation", "cost", "engagement", "top_sessions"]
    for report in expected_reports:
        assert report in results["reports"]
```
...
1. **Incremental Processing**
   ...
   - Only process new spans since last run
...
1. **S3 Upload**
   ```python
   def upload_to_s3(self, batch_dir, bucket, prefix):
       # Upload all files in batch_dir to S3
       pass
   ```

2. **Database Persistence**
   ```python
   def save_to_database(self, results, connection_string):
       # Insert metrics into PostgreSQL/MySQL
       pass
   ```
...
The `BatchReporter` class leverages the existing `SessionAnalyzer`:

- Uses `fetch_session_spans()` for data retrieval
- Uses `_extract_conversation_from_span()` for message parsing
- Uses `_estimate_cost()` for cost calculations
- Uses `MODEL_PRICING` for cost lookups

This ensures consistency with existing reports and reduces code duplication.

---

## Checklist for Implementation

- [ ] Create `batch_reporter.py` with BatchReporter class
- [ ] Implement `run_batch()` and `run_window()` methods
- [ ] Implement `_session_summary_report()`
- [ ] Implement `_conversation_export()`
- [ ] Implement `_cost_analysis_report()`
- [ ] Implement `_engagement_report()`
- [ ] Implement `_top_sessions_report()`
- [ ] Add CLI interface with argparse
- [ ] Add manifest generation
- [ ] Write tests
- [ ] Test with sample Phoenix data
- [ ] Document cron setup
